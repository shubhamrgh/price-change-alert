...
import com.trailify.service.notification.DeliveryResult;
...
public class PushService {
    ...
    public PushService(PushSubscriptionRepository repository,
                       @Value("${trailify.vapid.public-key}") String publicKey,
                       @Value("${trailify.vapid.private-key}") String privateKey,
                       @Value("${trailify.vapid.subject}") String subject) {
        this.repository = repository;
        this.vapidPublicKeyB64 = publicKey;
        if (publicKey == null || publicKey.isBlank() || privateKey == null || privateKey.isBlank()) {
            this.webPush = null;
            log.warn("Web Push is disabled: TRAILIFY_VAPID_PUBLIC_KEY and TRAILIFY_VAPID_PRIVATE_KEY are not configured");
            return;
        }
        try {
    ...
    public DeliveryResult notifyAll(String ownerId, String symbol, Market market, String message) {
        ...
        try {
            payload = mapper.writeValueAsString(java.util.Map.of(
                    "title", "Trailify: " + symbol.toUpperCase(Locale.ROOT),
                    "body", message,
                    "market", market.name(),
                    "url", "/#alerts"));
        } catch (Exception e) {
