...
public class AuthMailService {
    ...
    public AuthMailService(ObjectProvider<JavaMailSender> mailSenders,
                           @Value("${trailify.auth.email.enabled:false}") boolean enabled,
                           @Value("${trailify.auth.email.from:}") String from,
    ...
    public void sendMagicLink(String email, String link) {
        send(email, "Your Trailify sign-in link",
    ...
    public void sendPasswordReset(String email, String link) {
        send(email, "Reset your Trailify password",
